...
print("JAV Star name finder, V1.0, written by SangDo_Kim, a user in AVDBS.com.")
...
for i in range(len(lFileList)):
    ...
    if sCodeNum != sPreviousCodeNum:                        #방금 전 단계에서 이미 검색한 품번이 아닌 경우에만 검색 진행
        ...
        if iRequest > 0:                                    #구글의 차단을 회피하기 위해 잠시 대기
            print("     I no robot (I guess?) I'm idling for a while... 나는 로봇 아님(아마도?). 잠깐 노는 중...")
    ...
    if iTextLocation > 0:
        input("["+str(i+1)+"/"+str(len(lFileList))+"] 'Unusual Traffic' Text Location: "+str(iTextLocation)+".\n"
                 "Due to repeated web search, it seems Google think you are a robot.\n"+
                 "Please try it later (maybe in a few hours). Press Enter to exit.\n" +
                 "반복적인 웹 검색으로 인해 구글이 귀하를 로봇으로 생각하는 모양입니다.\n"+
                 "나중에 다시 시도하십시오(대략 몇 시간 후). 엔터를 눌러 종료하십시오.")
        sys.exit()
